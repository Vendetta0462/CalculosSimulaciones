...
donde $C_\text{max} = G M_\text{max} / c^2 R_{(M_\text{max})}$ es la compacidad de la estrella de masa máxima, cuyo límite superior se establece alrededor de $C_\text{max}= \lesssim 0.33$ tras evaluar implicaciones de observaciones recientes en ecuaciones de estado \cite{annalaMultimessengerConstraintsUltradense2022}. Consideramos esta definición de compacidad máxima teniendo en cuenta la condición de Harrison-Zeldovich-Novikov (\textbf{C6}) discutida en la sección \ref{sec:aceptabilidad}.
