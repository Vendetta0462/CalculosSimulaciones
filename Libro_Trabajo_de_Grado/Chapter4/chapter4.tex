...
El ajuste de estos parámetros se realiza imponiendo que el modelo reproduzca las propiedades empíricas de la materia nuclear en saturación, descritas en la sección \ref{sec:saturacion}, y las observaciones astrofísicas, descritas en la sección \ref{sec:obsNS}. Múltiples métodos son empleados para este ajuste: algunos estudios utilizan análisis bayesianos \cite{chenBuildingRelativisticMean2014, huangConstrainingRelativisticMean2024}, mientras que otros aplican técnicas de optimización utilizando herramientas de aprendizaje de máquina y redes neuronales \cite{guoInsightsNeutronStar2024}. Sin embargo, cada conjunto de parámetros que satisface estas restricciones nucleares produce una ecuación de estado diferente a altas densidades, generando predicciones distintas para las propiedades estelares observables como la masa máxima y el radio. Por esta misma razón, dentro de este mismo formalismo han sido propuestos una gran variedad de modelos de ecuaciones de estado, los cuales se filtran añadiendo cada vez más restricciones físicas \cite{dutraRelativisticMeanFieldHadronic2014}. Esta falta de unicidad en el espacio de parámetros refleja la incertidumbre en la extrapolación desde la densidad de saturación nuclear hacia los regímenes de densidad extrema presentes en el interior de las estrellas de neutrones.


El objetivo central de este capítulo es caracterizar sistemáticamente el espacio de parámetros del modelo, identificando los conjuntos que satisfacen las restricciones nucleares y que son consistentes con las observaciones astrofísicas actuales. Este análisis establece relaciones entre los parámetros microscópicos de la materia (\ref{eq:lagrangiano}) y las propiedades macroscópicas observables, permitiendo estudiar cómo las interacciones nucleares determinan la estructura de las estrellas de neutrones. En particular, en este trabajo queremos determinar el conjunto de parámetros que maximiza la masa estelar predicha, planteando límites superiores teóricos.
...
Para estudiar el impacto los parámetros del modelo en la ecuación de estado y las propiedades macroscópicas de las estrellas de neutrones, se presentan en la figura \ref{fig:eos_mass_radius_parametros} las ecuaciones de estado y las curvas masa-radio obtenidas al variar independientemente cada uno de los parámetros alrededor de los valores en (\ref{eq:params_glendenning}), manteniendo los otros parámetros fijos. Las ecuaciones de estado se presentan a partir del empalme con la ecuación de estado para la corteza, discutida en la sección \ref{sec:corteza}, el cual es el mismo en todos los casos. Con estas visualizaciones, podemos corroborar cómo afecta cada parámetro a la rigidez de la ecuación de estado, a la masa y al radio de las estrellas de neutrones. Se observa que únicamente el aumento del parámetro $A_\sigma$ produce un aumento en la rigidez de la ecuación de estado del modelo, mientras que los aumentos en $A_\omega$, $A_\rho$, $b$ y $c$ tienden a suavizar la ecuación de estado. Respecto a las masas de las estrellas, en los rangos considerados, los parámetros $A_\omega$ y $c$ muestran un impacto considerable al aumentar la masa máxima, mientras que $A_\sigma$ produce un aumento más moderado. Por otro lado, el parámetro $b$ produce una ligera disminución en la masa máxima al aumentar su valor, mientras que $A_\rho$ tiene un impacto casi nulo en la masa máxima. En cuanto a los radios, todos los parámetros considerados muestran un impacto significativo, siendo $A_\omega$ y $c$ los que producen los aumentos más pronunciados, mientras que $A_\rho$ y $b$ producen aumentos más moderados, y $A_\sigma$ produce una disminución moderada al aumentar su valor. \textbf{REALMENTE ES UN AUMENTO EN LA RIGIDEZ?}
...
\begin{figure}[h]
    ...
    \end{subfigure}
    \caption[Ecuaciones de estado y curvas masa-radio al variar los parámetros]{Ecuaciones de estado (izquierda) y curvas masa-radio (derecha) obtenidas al variar cada uno de los parámetros del modelo relativista de campo medio alrededor de los valores en (\ref{eq:params_glendenning}), manteniendo los demás parámetros fijos. (a) Variación de $A_\sigma$. (b) Variación de $A_\omega$. (c) Variación de $A_\rho$. (d) Variación de $b$. (e) Variación de $c$. \textbf{ORGANIZAR MEJOR LA VISUALIZACION}}
    \label{fig:eos_mass_radius_parametros}
\end{figure}
\clearpage
