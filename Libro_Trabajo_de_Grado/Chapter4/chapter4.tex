...
En este análisis se consideraron valores negativos para el parámetro $c$, siguiendo la elección en (\ref{eq:params_glendenning}). Podría pensarse que valores negativos de este parámetro permiten que la densidad de energía (\ref{eq:densidad_energia_adimensional}) no esté acotada inferiormente para valores altos del campo escalar $\sigma$, lo cual sería físicamente inaceptable. Glendenning \cite{glendenningCompactStarsNuclear2000} considera que esto es aceptable para una teoría efectiva de hadrones compuestos, ya que no es una teoría fundamental y, además, en el rango de densidades relevantes para las estrellas de neutrones la densidad de energía está bien comportada. Adicionalmente, como se verificó en la sección \ref{sec:autoconsistencia} (figura \ref{fig:soluciones_autoconsistentes}), la solución autoconsistente para la masa efectiva adimensional $\xsigma$ permanece siempre positiva y acotada entre 0 y 1, asegurando que la densidad de energía permanezca acotada inferiormente en todo el rango de densidades considerado.

\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{Figuras/variacion_params_A_i}
    \caption[Ecuaciones de estado y curvas masa-radio al variar los parámetros de acoplamiento]{Ecuaciones de estado en el rango de $n_0 = 0.157 \, \text{fm}^{-3}$ ($\approx 2.6\times 10^{14} \, \text{g cm}^{-3}$) hasta $1.5 \, \text{fm}^{-3}$ ($\approx 2.5\times 10^{15} \, \text{g cm}^{-3}$) (fila superior) y curvas masa-radio (fila inferior) obtenidas al variar los parámetros de acoplamiento mesón-nucleón alrededor de los valores en (\ref{eq:params_glendenning}), manteniendo los demás parámetros fijos. De izquierda a derecha se muestran las variaciones de $A_\sigma$, $A_\omega$ y $A_\rho$.}
    \label{fig:variacion_acoplamientos}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.7\linewidth]{Figuras/variacion_params_b_c}
    \caption[Ecuaciones de estado y curvas masa-radio al variar los parámetros de autointeracción escalar]{Ecuaciones de estado en el rango de $n_0 = 0.157 \, \text{fm}^{-3}$ ($\approx 2.6\times 10^{14} \, \text{g cm}^{-3}$) hasta $1.5 \, \text{fm}^{-3}$ ($\approx 2.5\times 10^{15} \, \text{g cm}^{-3}$) (fila superior) y curvas masa-radio (fila inferior) obtenidas al variar los parámetros de autointeracción del mesón escalar alrededor de los valores en (\ref{eq:params_glendenning}), manteniendo los demás parámetros fijos. De izquierda a derecha se muestran las variaciones de $b$ y $c$.}
...
\begin{figure}[h]
    ...
    \caption[Energía de enlace y fracción de neutrones para diferentes regiones]{Energía de enlace por nucleón (izquierda) y fracción de neutrones (derecha) para la configuración de mayor masa máxima en las regiones de la figura \ref{fig:comparacion_regiones}. La fracción de protones es entonces $n_p/n_B = 1 - n_n/n_B$.}
...
Finalmente, se realizó una comprobación de los criterios de aceptabilidad física para el caso particular de los parámetros en (\ref{eq:params_glendenning}) que puede consultarse en el Apéndice \ref{apendice:aceptabilidad}. La formulación del modelo dentro del marco de la teoría relativista de campo medio garantiza, por construcción, el cumplimiento de varias condiciones físicas importantes. Al derivarse de un lagrangiano covariante de Lorentz, el modelo asegura consistencia con los principios de relatividad especial, lo que implica que la velocidad del sonido en el medio es inferior a la velocidad de la luz ($v_s < c$) en el límite de altas densidades \cite{glendenningCompactStarsNuclear2000}. Además, la naturaleza vectorial de la interacción repulsiva previene inestabilidades mecánicas y violaciones de causalidad que suelen aparecer en modelos no relativistas a densidades supranucleares. Se verificó que una estrella de neutrones obtenida con este conjunto de parámetros cumple satisfactoriamente con todos los criterios de aceptabilidad física establecidos en la literatura \cite{ospinoRelativisiticNonpascalianFluid2025, hernandezAcceptabilityConditionsRelativistic2021}, con la excepción menor de la condición de estabilidad ante convección, la cual se satisface en el 97\% del interior estelar, considerándose un resultado aceptable. Se espera que las demás configuraciones encontradas en este estudio también respeten estos criterios, dado que comparten la misma estructura lagrangiana y sus ecuaciones de estado exhiben comportamientos similares en rigidez y monotonicidad al conjunto de referencia. No obstante, una verificación exhaustiva de estos criterios para todas las configuraciones obtenidas queda fuera del alcance del presente trabajo y se plantea como una perspectiva para investigaciones futuras.