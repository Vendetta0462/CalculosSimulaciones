...
En Hernández et al. \cite{hernandezAcceptabilityConditionsRelativistic2021} se recopilan una serie de criterios teóricos que un modelo estelar debe satisfacer para ser considerado físicamente admisible en el contexto de las estrellas de neutrones. Estos criterios incluyen condiciones sobre la estabilidad mecánica, la causalidad, la monotonía de la densidad y presión, entre otros. Para una estrella de neutrones estática y esféricamente simétrica, las condiciones \textbf{C1} a \textbf{C3} y \textbf{C8} se satisfacen automáticamente. Es decir, la métrica es positiva y regular en el interior de la estrella (\textbf{C1}), la presión y densidad son positivas y finitas en el centro (\textbf{C2}), la presión y densidad decrecen monotonamente desde el centro hacia la superficie estelar (\textbf{C3}) y la estrella es estable ante agrietamientos (\textbf{C8}) \cite{ramos-salamancaPhysicalAcceptabilityConditions2021}. Además, se ha demostrado que la condición en el índice adiabático (\textbf{C5}) es válida sólo en el límite Newtoniano \cite{moustakidisStabilityRelativisticStars2017}.

Como se observó en las figuras \ref{fig:materiaestelarbase} y \ref{fig:causalidadmateriabase}, la condición de energía $\rho \geq P$ (\textbf{C4}) y la condición de causalidad $v_s \leq c$ (\textbf{C6}) se cumplen para este modelo. La condición de estabilidad ante pulsaciones radiales (\textbf{C7}) se satisface si $dM/d\rho_c > 0$, de modo que sólo consideramos válidas las configuraciones estelares que cumplen esta condición, aún cuando se muestran en las curvas de masa-radio configuraciones inestables para tener mayor visión del comportamiento del modelo. Para el conjunto de parámetros empleados en la figura \ref{fig:materiaestelarbase}, la densidad de masa central máxima permitida por esta condición es $\rho_{m0} = 1.5 \times 10^{15} \, \text{g cm}^{-3}$, correspondiente a una masa máxima de $M_\text{max} = 2.33 \, \masasol$. Finalmente, la condición de estabilidad convectiva (\textbf{C9}) no se comprueba en este trabajo debido al alcance del estudio, pero se ha mostrado que otros modelos relativistas de campo medio, como el propuesto por Lalazissis et al. \cite{lalazissisNewParametrizationLagrangian1997}, no satisface la condición en todo el rango de densidades relevantes para estrellas de neutrones \cite{ramos-salamancaPhysicalAcceptabilityConditions2021}.
